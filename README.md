...
- [tokenscope](https://github.com/wartzar-bee/tokenscope) - Local, read-only CLI that parses Claude Code session logs to show where tokens go: model output vs. context re-sent every turn, with per-turn growth and cache-read breakdowns. ![Stars](https://img.shields.io/github/stars/wartzar-bee/tokenscope)
