...
- [agent-cost-guardrails](https://github.com/sapph1re/agent-cost-guardrails) - Hard budget enforcement and circuit breakers for multi-agent frameworks (CrewAI, AutoGen, LangGraph). Sets per-agent token/cost caps with automatic abort on breach; prevents runaway agent costs from reaching your billing. Available on [PyPI](https://pypi.org/project/agent-cost-guardrails/) and [npm](https://www.npmjs.com/package/agent-cost-guardrails). ![Stars](https://img.shields.io/github/stars/sapph1re/agent-cost-guardrails)
